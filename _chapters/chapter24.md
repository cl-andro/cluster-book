...
### 7.4 Case Study: Utilizing Compiler and Runtime Environments (Python, Go, Java, Kotlin, & Scala)
For compiler and interpreter containers (such as `python-runtime`, `golang-compiler`, `java-runtime`, `kotlin-compiler`, or `scala-compiler`), the default execution command in the recipe (e.g., `python3 --version`, `go version`, `java -version`, `kotlinc -version`, or `scala -version`) is configured as a simple startup sanity test. To leverage these containers for interactive development or building custom applications, developers can utilize two primary patterns:
...
2. **Mounting External Workspaces:**
   ...
   * **For Kotlin:**
     ```yaml
     mount_external:
         "/home/user/my-kotlin-app -> /app"
     run:
         "/opt/kotlin/bin/kotlinc /app/hello.kt -include-runtime -d /app/hello.jar"
         "/opt/kotlin/jre/bin/java -jar /app/hello.jar"
     ```
   * **For Scala:**
     ```yaml
     mount_external:
         "/home/user/my-scala-app -> /app"
     run:
         "/opt/scala/bin/scalac /app/hello.scala -d /app/classes"
         "/opt/scala/bin/scala -cp /app/classes hello"
     ```
